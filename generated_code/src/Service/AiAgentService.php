...
class AiAgentService
{
    ...
    public function runPrompt(string $prompt, array $options = []): array
    {
        ...
        while ($attempt <= self::MAX_RETRIES) {
            try {
                ...
                // Rückgabe eines Fehlerstatus
                return [
                    'status' => 'error',
                    'message' => 'AI-Agent konnte nach mehreren Versuchen nicht ausgeführt werden.',
        ...
        if (!$result) {
            return [
                'status' => 'error',
                'message' => 'Alle Versuche zur Ausführung des AI-Agenten sind fehlgeschlagen.',
                'statuses' => $this->agentStatusService->getStatuses(),
            ];
        }
        ...
        if ($result) {
            ...
            return [
                ...
                'ai_response' => $content,
                'statuses' => $this->agentStatusService->getStatuses(),
                // Weitere Details hier hinzufügen, z.B. files_created, deployment_instructions
            ];
        }

        // Sollte nicht erreicht werden, falls die Schleife beendet oder ein Fehler zurückgegeben wurde
        return [
            'status' => 'unknown_error',
            'message' => 'Unerwarteter Endzustand des Agenten-Aufrufs.',
            'statuses' => $this->agentStatusService->getStatuses(),
        ];
    }
}